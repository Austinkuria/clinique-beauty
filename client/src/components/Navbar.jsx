    ...
    return (
        <nav className={`sticky top-0 z-50 ${colors.navbarBg} ${colors.shadow} transition-all duration-300`}>
            <div className="container mx-auto px-4 md:px-6 lg:px-8 py-4 flex items-center justify-between">
                ...
                {/* Desktop Menu - Changed from hidden md:flex to make sure it's visible */}
                <div className="hidden lg:flex items-center space-x-6" style={{ display: 'flex', '@media (max-width: 768px)': { display: 'none' } }}>
                    <Link to="/products" 
                        className="text-gray-800 dark:text-gray-200 hover:text-pink-600 dark:hover:text-pink-400 transition-colors duration-200 font-medium"
                        style={{ color: 'inherit' }}>
                        Products
                    </Link>
                    <Link to="/cart" 
                        className="text-gray-800 dark:text-gray-200 hover:text-pink-600 dark:hover:text-pink-400 transition-colors duration-200 flex items-center gap-1 font-medium"
                        style={{ color: 'inherit' }}>
            ...
            >
                <div className="flex flex-col items-center space-y-4 py-4">
                    <Link to="/products" className={`${colors.navbarTextSecondary} ${colors.textHover}`} onClick={() => setIsOpen(false)}>
                        Products
                    </Link>
                    <Link to="/cart" className={`${colors.navbarTextSecondary} ${colors.textHover} flex items-center gap-1`} onClick={() => setIsOpen(false)}>
                        <ShoppingCartIcon className="w-5 h-5" /> Cart
                    </Link>
                    <Link to="/profile" className={`${colors.navbarTextSecondary} ${colors.textHover} flex items-center gap-1`} onClick={() => setIsOpen(false)}>
                        <UserIcon className="w-5 h-5" /> Profile
                    </Link>
                    <Link to="/login" className={`${colors.navbarTextSecondary} ${colors.textHover}`} onClick={() => setIsOpen(false)}>
                        Login
                    </Link>
                    <button
